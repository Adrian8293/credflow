...
export async function uploadDocumentFile(documentId, providerId, file) {
  ...
  if (uploadError) {
    console.error('[uploadDocumentFile] storage upload failed:', JSON.stringify(uploadError))
  ...
  if (updateError) {
    console.error('[uploadDocumentFile] DB update failed:', JSON.stringify(updateError))
    throw updateError
  }

  safeAudit('Document', 'File Attached', `${file.name} → doc ${documentId}`, documentId)
