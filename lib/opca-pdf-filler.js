...
export async function fillOpcaPdf({ templateBuffer, profile, initials }) {
  ...
  for (const { field_id, value } of fieldValues) {
    try {
      ...
      // Skip rich text fields — pdf-lib cannot read or write them
      if (fieldType === 'PDFTextField') {
        try {
          field.setText(value || '')
        } catch (rtErr) {
          // Rich text field — leave blank, provider fills manually
          console.warn(`[opca-pdf-filler] Rich text field skipped "${field_id}": ${rtErr.message}`)
        }
  ...
  // Do NOT flatten — rich text fields crash pdf-lib's flatten().
  // The PDF stays editable so the provider can complete any remaining fields.
